...
struct RTTSetRIPASFuzz {
    ...
    /* We need to tamper with the REC to apply the following */
    ripas_state: u8,
    ripas_flags: u64,
}
...
fuzz_target!(|data: RTTSetRIPASFuzz| -> Corpus {
    ...
    unsafe {
        ...
        /* Modify rec to store ripas state and flags. */
        rec.set_ripas(data.base, data.top, data.ripas_state, data.ripas_flags);
    }
